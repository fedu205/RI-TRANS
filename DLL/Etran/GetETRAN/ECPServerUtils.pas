...
uses  SysUtils, Windows, Forms, Variants, StrUtils, Dialogs, Registry, Messages, IEtranSysservice,
...
function GetDoument(id: integer): string;
...
begin
  try
    ...
    Q.ConnectionString := 'Persist Security Info=True;Provider=SQLOLEDB.1;User ID=sa;Password=QAZ123qaz;Initial Catalog=Lis_Etran;Data Source=(local);';

    Q.SQL.Add('declare @etran_doc_id int');
    Q.SQL.Add('set @etran_doc_id = ' + id.ToString);
    Q.SQL.Add('');
    Q.SQL.Add('declare @res varchar(max)');
    Q.SQL.Add('DECLARE @B VARBINARY(MAX), @A varchar(max)');
    Q.SQL.Add('SELECT @B = doc_blob from ETRAN_ECP_DOC where etran_doc_id = @etran_doc_id');
    Q.SQL.Add('select @a = cast('''' as  xml).value(''xs:base64Binary(sql:variable("@B"))'', ''varchar(max)'')');
    Q.SQL.Add('');
    Q.SQL.Add('select	@res =');
    Q.SQL.Add('''<SetEGK>'' +');
    Q.SQL.Add('''<Doc_id value="'' + cast(isnull(invoiceid, -9) as varchar(max)) + ''"/>''+');
    Q.SQL.Add('''<SD_TYPE_ID>''+ cast(sd_type_id as varchar(max)) + ''</SD_TYPE_ID>''+');
    Q.SQL.Add('''<EGK>'' + @a + ''</EGK>''+');
    Q.SQL.Add('''<FILEEXT>'' + fileext + ''</FILEEXT>''+');
    Q.SQL.Add('''<DESCR>'' + replace(descr,''.pdf'','''') + ''</DESCR>''+');
    Q.SQL.Add('''</SetEGK>''');
    Q.SQL.Add('from	etran_ecp_doc inner join etran_ecp on etran_ecp_doc.etran_ecp_id = etran_ecp.etran_ecp_id');
    Q.SQL.Add('where	etran_doc_id = @etran_doc_id');
    Q.SQL.Add('');
    Q.SQL.Add('select @res res');
    Q.Open;

    Result := Q.FieldByName('res').AsString;
...
begin
  ...
  ss.Add('1'); ss.SaveToFile('C:\GetEtran\log\log.txt');

  file_blob.SaveToFile('C:\GetEtran\log\blob22.txt');

  ss.Add('2'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  ...
  ss.Add('3'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  ...
  ss.Add('4'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  hCert := nil;
  ...
  ss.Add('5'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  ...
  ss.Add('6'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  ...
  ss.Add('7'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  ...
  begin
    message_error := 'Ошибка при подписании данных (1)!' + SysErrorMessage(GetLastError) + ' ' + GetLastError.ToString;
    Result := False;
    Exit;
  end;

  ss.Add('8'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  // Подписываем файл
  ...
  if (not (CryptSignMessage(@SigParams,
                        ...
                        cbSignedMessageBlob))) then
  begin
    message_error := 'Ошибка при подписании данных (2)!' + #10 + SysErrorMessage(GetLastError) + ' ' + GetLastError.ToString;
    Result := False;
    Exit;
  end;

  ss.Add('9'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  pStreamOut.SetSize(cbSignedMessageBlob);
  pStreamOut.Seek(0,0);

//  pStreamOut.SaveToFile('blob2.txt');

  MessageBASE64 := TStringStream.Create;
  EncodeStream(pStreamOut, MessageBASE64);

  MessageBASE64.SaveToFile('C:\GetEtran\log\blob.sig');

  res := MessageBASE64.DataString;
  MessageBASE64.Free;
  pStreamOut.Free;

  ss.Add('10'); ss.SaveToFile('C:\GetEtran\log\log.txt');
  ss.Free;
  result_xml := res;
