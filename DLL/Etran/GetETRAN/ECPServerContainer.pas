...
//  fmECPServerMain.Memo1.Lines.Add('Connect (2)= ' + Event.Channel.ChannelInfo.ClientInfo.IpAddress + ':' + Event.Channel.ChannelInfo.ClientInfo.ClientPort);
  ...
  TThread.Synchronize(nil,
    procedure
    begin
      fmEtranMain.AddConnectionToList(TIdTCPConnection(Event.Connection), Event.Channel);
    end);
end;
    ...
    TThread.Synchronize(nil,
      ...
      begin
        for i := 0 to fmEtranMain.ListBox1.Items.Count-1 do begin
          if TDSTCPChannel(fmEtranMain.ListBox1.Items.Objects[i]).GetConnection = Event.Connection then begin
            fmEtranMain.ListBox1.Items.Delete(i);
            break;
          end;
        end;
      end);
