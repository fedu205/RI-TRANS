...
begin
  ...
  end else begin //  урсы пробиты в подр€дчике
    case shaping_rate_currency_id of
...
procedure TfmFactIncCard.ShowFactInfo; // подключить дл€ показа информацию по факту
...
begin
  ...
  Q_Fact.Open;
  DS_Fact.DataSet := Q_Fact; // нужен TDataSource дл€ прив€зки видимых полей в форме

  // расстановка видимых данных в соответствии с пол€ми запроса
  ...
  // возможно список не понадобитс€ в форме или расчете, но пока оставил
  ...
  Query_ListRate.Open;
  // расстановка видимых данных в соответствии с пол€ми FSelfFactDataset, вз€тыми
  ...
  // извлечение ƒополнительных сборов по оплате проезда + сначала нужно найти shapig_rate_id
  ...
  // извлечение ƒополнительных сборов по оплате проезда
...
begin
  Screen.Cursor := crHourglass;
  fmOther.Show;
  fmOther.Label1.Caption := 'ѕроверка ставок по загрузке вагонов/контейнеров...';
  fmOther.Repaint;

  fmOther.Label1.Caption := '«апуск сервера автоматизации...';
  fmOther.Repaint;
...
//  exApp.Visible := True;
  exWks.Range['B1'] := 'ќтпр'; exWks.Range['C1'] := 'ѕриб';
  for i:=0 to 79 do
